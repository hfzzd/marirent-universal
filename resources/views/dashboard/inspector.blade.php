...
@php
    ...
    $totalMyInspections = \App\Models\Inspection::where('inspector_id', $uid)->count();
    $myInspectionsThisMonth = \App\Models\Inspection::where('inspector_id', $uid)->whereMonth('created_at', now()->month)->count();
    $myDamageFindings = \App\Models\Inspection::where('inspector_id', $uid)->get()->filter(fn($i) => $i->hasDamages())->count();

    // Booking yang butuh Inspeksi Awal: sudah confirmed, belum ada inspeksi pre_rental
    $needInitial = \App\Models\Booking::with(['user', 'vehicle', 'category'])
    ...
    // Booking yang butuh Inspeksi Akhir: ongoing, belum ada inspeksi post_rental
    $needFinal = \App\Models\Booking::with(['user', 'vehicle', 'category'])
    ...
    $recentMyInspections = \App\Models\Inspection::with(['booking', 'vehicle', 'item'])
    ...
    <div class="relative px-6 py-7 md:px-8 md:py-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
        <div>
            ...
            <p class="text-emerald-100/80 text-xs md:text-sm mt-1 max-w-xl">
                Pantau tugas inspeksi awal & akhir unit kendaraan maupun barang sewa lainnya.
            </p>
            <div class="flex flex-wrap items-center gap-3 mt-4">
                <div class="flex items-center gap-2 bg-black/20 backdrop-blur-md rounded-xl px-3.5 py-1.5 border border-white/10 text-white text-xs">
                    ...
                    <span><strong>{{ $needInitial->count() }}</strong> Butuh Inspeksi Awal</span>
                </div>
                <div class="flex items-center gap-2 bg-black/20 backdrop-blur-md rounded-xl px-3.5 py-1.5 border border-white/10 text-white text-xs">
                    <i class="fas fa-flag-checkered text-amber-300"></i>
                    <span><strong>{{ $needFinal->count() }}</strong> Butuh Inspeksi Akhir</span>
                </div>
            </div>
        </div>
    ...
    <div class="stat-card glass-card rounded-2xl p-5 border border-sky-100/50 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-black text-navy-800 mt-1">{{ $totalMyInspections }}</h3>
    ...
    <div class="stat-card glass-card rounded-2xl p-5 border border-sky-100/50 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-black text-navy-800 mt-1">{{ $myInspectionsThisMonth }}</h3>
    ...
    <div class="stat-card glass-card rounded-2xl p-5 border border-sky-100/50 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-black text-navy-800 mt-1">{{ $myDamageFindings }}</h3>
    ...
    <div class="stat-card glass-card rounded-2xl p-5 border border-sky-100/50 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-black text-navy-800 mt-1">{{ $needInitial->count() + $needFinal->count() }}</h3>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    ...
    <div class="glass-card rounded-2xl overflow-hidden border border-sky-100/50 shadow-sm">
        <div class="px-6 py-4 border-b border-sky-100/60 flex items-center justify-between">
            ...
            <span class="badge badge-blue">{{ $needInitial->count() }} booking</span>
        </div>
        <div class="divide-y divide-gray-100 max-h-96 overflow-y-auto">
            @forelse($needInitial as $b)
    ...
    <div class="glass-card rounded-2xl overflow-hidden border border-sky-100/50 shadow-sm">
        <div class="px-6 py-4 border-b border-sky-100/60 flex items-center justify-between">
            ...
            <span class="badge badge-yellow">{{ $needFinal->count() }} booking</span>
        </div>
        <div class="divide-y divide-gray-100 max-h-96 overflow-y-auto">
            @forelse($needFinal as $b)
            <div class="px-5 py-3.5 hover:bg-sky-50/30 transition flex items-center justify-between gap-3">
                <div class="min-w-0">
                    <p class="font-bold text-navy-800 text-[13px] truncate">{{ $b->vehicle?->name ?? ($b->category?->name ?? 'Unit Sewa') }}</p>
...
<div class="glass-card rounded-2xl overflow-hidden shadow-sm border border-sky-100/50">
    <div class="px-6 py-4 border-b border-sky-100/60 flex items-center justify-between">
        <div>
            ...
            <p class="text-[11px] text-gray-400">6 pemeriksaan terakhir yang Anda catat.</p>
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-xs">
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($recentMyInspections as $ins)
                <tr class="hover:bg-sky-50/30 transition-colors">
                    ...
                    <td class="py-3.5 px-6 text-center">
                        @if($ins->hasDamages()) <span class="badge badge-red">{{ count(array_merge($ins->damage_items ?? [], is_array($ins->damages) ? $ins->damages : [])) }} Temuan</span>
