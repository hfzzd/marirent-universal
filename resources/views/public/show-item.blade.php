...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="mb-6">
        <a href="{{ route('products', ['category' => $item->category->slug ?? '']) }}" class="text-primary-600 hover:text-primary-700 text-sm"><i class="fas fa-arrow-left mr-1"></i> Kembali ke {{ $config['label'] }}</a>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                <div class="h-80 bg-gradient-to-br from-primary-50 to-primary-100 flex items-center justify-center">
                    ...
                    @else
                        <i class="fas {{ $config['icon'] }} text-primary-300 text-8xl"></i>
                    @endif
                </div>
                <div class="p-6">
                    <div class="flex items-start justify-between mb-4">
                        <div>
                            <span class="bg-primary-50 text-primary-700 px-3 py-1 rounded-full text-xs font-medium">{{ $config['label'] }}</span>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <a href="{{ route('public.item', [$type, $r->slug]) }}" class="group border border-gray-100 rounded-xl p-3 hover:border-primary-300 transition">
                        <div class="h-20 bg-primary-50 rounded-lg flex items-center justify-center mb-2 overflow-hidden">
                            @if($r->image)
                                <img src="{{ asset('storage/' . $r->image) }}" class="w-full h-full object-cover" alt="{{ $r->name }}">
                            @else
                                <i class="fas {{ $config['icon'] }} text-primary-300 text-2xl"></i>
                            @endif
                        </div>
                        <p class="text-[12px] font-semibold text-navy-800 group-hover:text-primary-600 leading-tight">{{ Str::limit($r->name, 28) }}</p>
                        <p class="text-[11px] text-primary-600 font-medium">Rp {{ number_format($r->daily_price, 0, ',', '.') }}/hari</p>
                    </a>
                    @endforeach
                </div>
            </div>
        ...
        <div>
            <div class="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
                <div class="mb-6">
                    ...
                    <p class="text-3xl font-bold text-primary-600">Rp {{ number_format($item->daily_price, 0, ',', '.') }}</p>
