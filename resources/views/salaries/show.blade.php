    ...
    <div class="bg-white rounded-2xl shadow-sm p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="space-y-3 text-sm">
                ...
                <div class="border-t pt-3 flex justify-between"><span class="font-bold text-navy-800">Total Gaji</span><span class="font-bold text-xl text-primary-600">Rp {{ number_format($salary->total_salary,0,',','.') }}</span></div>
            </div>
            <div class="space-y-3 text-sm">
                ...
                @if($salary->invoice)
                <div class="flex justify-between"><span class="text-navy-500">Invoice</span><a href="{{ route('invoices.show', $salary->invoice) }}" class="font-medium text-primary-600">{{ $salary->invoice->invoice_number }}</a></div>
                @endif
                @if($salary->notes)
