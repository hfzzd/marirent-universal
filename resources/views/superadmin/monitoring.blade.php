    ...
    <div x-show="tab === 'scheduler'" x-cloak>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div class="stat-card glass-card rounded-2xl p-5 border border-sky-100/60 shadow-sm">
                ...
                <h3 class="text-xl font-black text-navy-800 mt-1">Rp {{ number_format($summary['outstanding_all_amount'], 0, ',', '.') }}</h3>
                    ...
                    <div class="grid grid-cols-7 gap-1.5">
                        ...
                        <div class="rounded-xl border {{ $isToday ? 'border-sky-400 ring-2 ring-sky-100' : ($inMonth ? 'border-gray-100' : 'border-gray-50 opacity-40') }} bg-white min-h-[92px] p-1.5 relative {{ $hasOverdue ? 'bg-red-50/40' : '' }}">
                            ...
                            <div class="mt-1 space-y-1">
                                ...
                                <a href="{{ route('bookings.show', $e['id']) }}" title="{{ $e['item'] }} - Rp {{ number_format($e['amount'],0,',','.') }}"
                                   class="block truncate text-[9px] leading-tight px-1.5 py-1 rounded-md font-semibold transition hover:opacity-80 {{ $e['overdue'] ? 'bg-red-100 text-red-700' : ($e['status'] === 'partial' ? 'bg-amber-100 text-amber-700' : ($e['status'] === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-50 text-amber-600')) }}">
                                    {{ $e['item'] }}
                                </a>
                                @endforeach
