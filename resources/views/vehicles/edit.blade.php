...
@section('page-title', 'Edit Kendaraan - ' . $vehicle->name)
@section('content')
        ...
        <form method="POST" action="{{ route('vehicles.update', $vehicle) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="hourly_price" value="{{ old('hourly_price', $vehicle->hourly_price) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-navy-700 mb-1">Harga per Hari (Rp) *</label>
                    <input type="number" name="daily_price" value="{{ old('daily_price', $vehicle->daily_price) }}" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-navy-700 mb-1">Harga per Minggu (Rp)</label>
                    <input type="number" name="weekly_price" value="{{ old('weekly_price', $vehicle->weekly_price) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-navy-700 mb-1">Harga per Bulan (Rp)</label>
                    <input type="number" name="monthly_price" value="{{ old('monthly_price', $vehicle->monthly_price) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-navy-700 mb-1">Harga + Driver/Hari (Rp)</label>
                    <input type="number" name="with_driver_daily_price" value="{{ old('with_driver_daily_price', $vehicle->with_driver_daily_price) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm">
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="file" name="image" accept="image/*" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm">
                </div>
                <div class="md:col-span-2 flex items-center gap-6">
                    <label class="flex items-center text-sm">
                        <input type="checkbox" name="with_driver" value="1" {{ old('with_driver', $vehicle->with_driver) ? 'checked' : '' }} class="mr-2 rounded text-primary-600"> Sedia dengan Driver
                    </label>
                    <label class="flex items-center text-sm">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $vehicle->is_active) ? 'checked' : '' }} class="mr-2 rounded text-primary-600"> Aktif
                    </label>
                </div>
            </div>
